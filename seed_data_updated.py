...
def seed_courses():
    ...
    for course_data in all_courses:
        ...
        # Add course to categories
        if course_data["programming_language"] == "Python":
            categories = [1, 10]  # لغات البرمجة، البرمجة للمبتدئين
        elif course_data["programming_language"] == "JavaScript":
            categories = [1, 2]  # لغات البرمجة، تطوير الويب
        elif course_data["programming_language"] == "Dart":
            categories = [1, 3]  # لغات البرمجة، تطوير الموبايل
        elif course_data["programming_language"] == "SQL":
            categories = [4]  # قواعد البيانات
        else:
            categories = [1]  # لغات البرمجة
            
        for category_id in categories:
            db.session.execute(db.text(
                f"INSERT INTO course_categories (course_id, category_id) VALUES ({course.id}, {category_id})"
            ))
        ...
        for i in range(1, num_sections + 1):
            ...
            for j in range(1, num_lessons + 1):
                lesson = CourseLesson(
                    ...
                    order_index=j,
                    video_url=f"https://www.youtube.com/watch?v={course_data['youtube_video_id']}&list={course_data['youtube_playlist_id']}&index={j}"
                )
    ...
    quizzes = [
        {
            ...
            "questions": [
                ...
                },
                {
                    ...
                    "question_type": "multiple_choice",
                    "code_snippet": "x = 5 / 2\ntype(x)",
                    "options": json.dumps(["int", "float", "double", "decimal"]),
            ...
            "questions": [
                {
                    ...
                    "options": json.dumps(["[2, 4, 6, 8, 10]", "[6, 8, 10]", "[3, 4, 5]", "[1, 2, 3, 4, 5]"]),
                ...
                },
                {
                    ...
                    "options": json.dumps(["null", "undefined", "object", "number"]),
            ...
            "questions": [
                {
                    ...
                    "question_type": "multiple_choice",
                    "options": json.dumps(["SELECT users FROM *;", "SELECT * FROM users;", "SELECT ALL FROM users;", "SELECT COLUMNS FROM users;"]),
                ...
                },
                {
                    ...
                    "question_type": "multiple_choice",
                    "options": json.dumps(["ADD INTO users VALUES (1, 'John');", "INSERT VALUES (1, 'John') INTO users;", "INSERT INTO users VALUES (1, 'John');", "UPDATE users SET VALUES (1, 'John');"]),
...
المستقبل ينتمي للمبرمجين الذين يمكنهم الجمع بين مهارات البرمجة التقليدية وفهم تقنيات الذكاء الاصطناعي لإنشاء الجيل التالي من التطبيقات الذكية.""",
    ...
    for article_data in articles:
        ...
        # Add article to categories
        if article.title.find("البرمجة") != -1:
            categories = [1, 10]  # لغات البرمجة، البرمجة للمبتدئين
        elif article.title.find("JavaScript") != -1:
            categories = [1, 2]  # لغات البرمجة، تطوير الويب
        elif article.title.find("الذكاء الاصطناعي") != -1:
            categories = [1, 5]  # لغات البرمجة، الذكاء الاصطناعي
        else:
            categories = [1]  # لغات البرمجة
            
        for category_id in categories:
            db.session.execute(db.text(
                f"INSERT INTO article_categories (article_id, category_id) VALUES ({article.id}, {category_id})"
            ))
    ...
    for i in range(1, 14):  # For each course
        ...
        for j in range(num_ratings):
            ...
            course_rating = CourseRating(
                ...
                course_id=i,
                rating=rating,
                review_text=f"تقييم للدورة {i} من المستخدم {user_id}. {'دورة ممتازة ومفيدة جداً!' if rating >= 4 else 'دورة جيدة ولكن تحتاج إلى بعض التحسينات.'}"
            )
    ...
    for user_id in range(1, 6):  # For each user
        ...
        for i in range(num_courses):
            ...
            course_progress = UserCourseProgress(
                ...
                completed_lessons=json.dumps([random.randint(1, 10) for _ in range(random.randint(1, 5))]),
                status='in_progress'
            )
